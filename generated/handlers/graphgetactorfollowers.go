...
func (h *Handlers) HandleGraphGetActorFollowers(e echo.Context) error {
	var input GraphGetActorFollowersInput
	if err := e.Bind(&input); err != nil {
		logger := h.server.Logger().With("handler", "HandleGraphGetActorFollowers")
		logger.Error("error binding request", "err", err)
		return echo.NewHTTPError(http.StatusInternalServerError, "Internal Server Error")
	}

	output, err := h.server.HandleGraphGetActorFollowers(e, &input)
	if err != nil {
		return err
	}
